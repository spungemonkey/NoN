...
public class GUI_Button : MonoBehaviour {
	...
	void OnGUI(){
		...
		if (!trigger.activeInHierarchy) {
						if (GUI.Button (new Rect (butPos, Screen.height / 2, butImg.width, butImg.height), butImg) && 
						!trigger.activeInHierarchy ||
						GetButtonDown(KeyCode.JoystickButton0) &&	//untested
						...
						GetButtonDown(KeyCode.JoystickButton1)) { 	//untested
								audio.clip = ac2;
								audio.Play ();
						}
